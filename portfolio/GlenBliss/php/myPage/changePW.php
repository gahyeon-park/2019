...
if(sqlDB($memberConn, $memberResult, $sql) == -1) {
    ...
    echo "<script>alert('비밀번호 변경에 실패하였습니다.'); window.open('../../html/main/main.php', '_self');</script>";
}
else {
    /* 데이터 베이스 닫기 */
    closeDB($memberConn, $memberResult);
    echo "<script>alert('비밀번호가 정상적으로 변경되었습니다.'); window.open('../../html/main/main.php', '_self');</script>";
}
